...
namespace DiveSpots.Web.Core.TokenHandling
{
    public class TokenGenerator : ITokenGenerator
    {
        ...
        private async Task<object> GenerateTokenAsync(string username)
        {
            ...
            {
                claims.Add(new Claim(ClaimTypes.Role, Rolenames.Member));
            }
            ...
            if (await userManager.IsInRoleAsync(user, Rolenames.Moderator))
            {
                claims.Add(new Claim(ClaimTypes.Role, Rolenames.Moderator));
            }

            var secretKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(configuration.TokenSecurityKey));

            var token = new JwtSecurityToken(
                issuer: nameof(DiveSpots),
                audience: "everybody",
                claims: claims,
